...
class Condition:
    ...
    def __init__(self, default_db='postgres', **kwargs):
        if 'table_year' in kwargs:
            self.table_year = kwargs.pop('table_year')
        ...
        for expr, value in kwargs.items():
            ...
            if isinstance(value, F):
                f_obj = value
                sql_format_parts.append(
                    f'{field} {operation_str} {f_obj.sql_format}')
            elif isinstance(value, list):
                vars_list = value
                sql_format_parts.append(
                    f'{field} {operation_str} \
                        ({", ".join(["%s"]*len(vars_list))})')
                self.query_vars += vars_list
            else:
                # if default_db == 'adabas':
                value = Condition._clean_fields(value)
    ...
    def _clean_fields(value):
        if isinstance(value, str):
            ...
            # 2014-07-14 15:57:40.767
            value = f"'{value.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]}'"
        elif isinstance(value, dt.date):
            value = f"'{value.strftime('%Y-%m-%d')}'"
        elif str(value.__class__) == "<class 'datetime.date'>":
            value = f"'{value.strftime('%Y-%m-%d')}'"
    ...
    def _merge_with(self, other, logical_operator='AND'):
        ...
        condition_resulting.sql_format = f"({self.sql_format}) {logical_operator} \
            ({other.sql_format})"
...
def filter(obj_list: List, fields: List):
    aux_list = list()
    for obj in obj_list:
        key_vals = [str(getattr(obj, field)) for field in fields]
        aux_list.append(key_vals)

    set_list = set([','.join(l) for l in aux_list])
    return [l.split(',') for l in set_list]


def group_by(list_dict: List, filters):
    aux_list = dict()
    for item in list_dict:
        key_vals = [str(getattr(item, filter)) for filter in filters]
        key = ','.join(key_vals)
        try:
            aux_list[key].append(item)
        except KeyError:
            aux_list[key] = [item]
...
def get_values_list(list_dict: List, filters):
    aux_list = []
    for item in list_dict:
        for filter in filters:
            aux_list.append(getattr(item, filter))
